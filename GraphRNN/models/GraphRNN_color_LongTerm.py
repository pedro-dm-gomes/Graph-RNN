...
from graphrnn_cell_impl import *
...
class AdvancedGraphRNN(object):
	...
	# No color
        self.inputs = tf.placeholder(tf.float32, [batch_size, seq_length, num_points, 6])
        ...
        context_frames = int(seq_length/2) # 6 frames
        ...
        for i in range(int(context_frames) ):
            ...
            input_frame_points, input_frame_color = tf.split(input_frame, 2, axis = 2)
            
            xyz0 = input_frame_points
            color0 = input_frame_color
                
            
            print("\n === Downsample Module 1  ====") 
            xyz1, color1, feat1, states1, _, _ = sample_and_group(int(sampled_points_down1), radius=1.0+1e-8, nsample= 4, xyz=xyz0,  color=color0, features=None, states = None, knn=True, use_xyz=False)            

                        
            print("\n === CELL 1  Point-Features ====")
            with tf.variable_scope('encoder_1', reuse=tf.AUTO_REUSE) as scope:
            	out_1 = cell_feat_1((xyz1, color1, None, None))
            ...
            with tf.variable_scope('encoder_2', reuse=tf.AUTO_REUSE) as scope:
            	out_2 = cell_feat_2((f_xyz1, f_color1, f_feat1, None))
            ...
            with tf.variable_scope('encoder_3', reuse=tf.AUTO_REUSE) as scope:
            	out_3 = cell_feat_3((f_xyz2, f_color2, f_feat2, None))
            ...
            with tf.variable_scope('decoder_1', reuse=tf.AUTO_REUSE) as scope:
            	global_state1 = graph_cell1( (f_xyz3, f_color3, f_feat3, None, time), global_state1)
            ...
            xyz2, color2, feat2, states2, _, _ = sample_and_group(int(sampled_points_down2), radius=1.0+1e-20, nsample= 4 , xyz=s_xyz1,  color=s_color1, features=s_feat1, states = s_states1, knn=True, use_xyz=False)                
            ...
            with tf.variable_scope('decoder_2', reuse=tf.AUTO_REUSE) as scope:
                global_state2 = graph_cell2( (xyz2, color2, feat2, states2, time), global_state2)
            ...
            xyz3, color3, feat3, states3, _, _ = sample_and_group(int(sampled_points_down3), radius=4.0+1e-20, nsample= 4, xyz=s_xyz2,  color=s_color2, features=s_feat2, states = s_states2, knn=True, use_xyz=False)                
            ...
            with tf.variable_scope('decoder_3', reuse=tf.AUTO_REUSE) as scope:
                global_state3 = graph_cell3( (xyz3, color3, feat3, states3, time), global_state3)
	    ...
	    # I NEED TO MAKE A COLOR PREDICTION
            with tf.variable_scope('fcc', reuse=tf.AUTO_REUSE) as scope:
                predicted_motion_colors = tf.layers.conv1d(inputs=l0_feat, filters=128, kernel_size=1, strides=1, padding='valid', data_format='channels_last', activation=tf.nn.relu, name='fcc1')
                predicted_motion_colors = tf.layers.conv1d(inputs=predicted_motion_colors, filters=3, kernel_size=1, strides=1, padding='valid', data_format='channels_last', activation=None, name='fcc2')         
                
                         
            predicted_motions.append(predicted_motion)
            predicted_motions_colors.append(predicted_motion_colors)
            
            predicted_frame_points = xyz0 + predicted_motion
            predicted_frame_colors = color0 + predicted_motion_colors
            predicted_frame = tf.concat((predicted_frame_points,predicted_frame_colors), axis =2)
            predicted_frames.append(predicted_frame)
            
                 
            #input is prediction
            input_frame = predicted_frame
        ...
        for i in range(int(seq_length/2)):
            	...
            	frame_points, frame_color = tf.split(frame, 2, axis = 2)
            	predicted_frame_points, predicted_frame_color = tf.split(predicted_frame, 2, axis = 2)  
