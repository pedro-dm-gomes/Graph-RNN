"""
Models for long-term prediction without color
...
Info on model: This model uses hierchical architecture for long term-prediction
"""   
class AdvancedGraphRNN(object):
    def __init__(self, batch_size, seq_length, num_points=256, num_samples=8, knn=True, alpha=1.0, beta=1.0,alpha_color=0.0, beta_color=0.0, learning_rate=1e-5, max_gradient_norm=5.0, activation =None, is_training=False):
        ...
        context_frames = int(seq_length/2)
...
Info on model: This model has a basic architecture (without downsampling) for long term-prediction
"""           	
     	

class BasicGraphRNN(object):
    def __init__(self, batch_size, seq_length, num_points=256, num_samples=8, knn=True, alpha=1.0, beta=1.0,alpha_color=0.0, beta_color=0.0, learning_rate=0.001, max_gradient_norm=5.0, activation =None, is_training=False):
        ...
        for i in range(int(context_frames) ):
            ...
            with tf.variable_scope('graphrnn_1', reuse=tf.AUTO_REUSE) as scope:
            	global_state1 = graph_cell1( (f_xyz3, None, f_feat3, None, time), global_state1)
            	s_xyz1, s_color1, s_feat1, s_states1, time, _  = global_state1
            	print("s_xyz1",s_xyz1)
            	print("s_feat1",s_feat1)
            	print("s_color1",s_color1)
            	print("s_states1",s_states1)
            ...
            with tf.variable_scope('graphrnn_2', reuse=tf.AUTO_REUSE) as scope:
                global_state2 = graph_cell2( (xyz2, None, feat2, states2, time), global_state2)
                s_xyz2, s_color2, s_feat2, s_states2, time, _ = global_state2
                print("s_xyz2",s_xyz2)
                print("s_feat2",s_feat2)
                print("s_color2",s_color2)
                print("s_states2",s_states2)
            ...
            with tf.variable_scope('graphrnn_3', reuse=tf.AUTO_REUSE) as scope:
                global_state3 = graph_cell3( (xyz3, None, feat3, states3, time), global_state3)
                s_xyz3, s_color3,s_feat3, s_states3, time, _  = global_state3
                print("s_xyz3",s_xyz3)
                print("s_feat3",s_feat3)
                print("s_color3",s_color3)
                print("s_states3",s_states3)                
