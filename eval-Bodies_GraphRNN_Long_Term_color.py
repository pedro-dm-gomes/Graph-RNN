...
from models.GraphRNN_color_LongTerm import AdvancedGraphRNN as Model
from datasets.bodys_eval import Bodys as Dataset_Bodies_eval
...
parser.add_argument('--ckpt-step', type=int, default=1500000, help='Checkpoint step [default: 200000]')
...
parser.add_argument('--log-dir', default='outputs/GraphRNN_LongTerm_Color', help='Log dir [outputs/bodys-pointrnn]')
...
test_dir = os.path.join(args.log_dir, 'test-examples_Bodies')
if not os.path.exists(test_dir):
    os.makedirs(test_dir)

log = open(os.path.join(args.log_dir, 'test_Bodies.log'), 'w')
log_frame = open(os.path.join(args.log_dir, 'test_Bodies_frames.log'), 'w')
...
def run_test_sequence(sequence_nr):
    ...
    print("batch_points ", batch_points.shape)
    batch =np.concatenate((batch_points,batch_color), axis=3)
    print("batch_data ", batch.shape)
    print("batch_data ", batch.shape) # 1,12,4000,6
    ...
    log.flush()
    print('[%s]\tTEST:[%10d:]\t[CD,EMD]:\t%.12f\t%.12f\n'%(str(datetime.now()), sequence_nr, cd, emd))
    ...
    downsample_frames, batch_color = np.split(downsample_frames, 2, axis =2)
    npy_path= os.path.join(test_dir, 'gdt_test_' +str(sequence_nr) )
    ...
    pc_prediction, batch_color = np.split(pc_prediction, 2, axis =2)
    npy_path= os.path.join(test_dir, 'pdt_test_' +str(sequence_nr) )
...
with tf.Session() as sess:
    ...
    nr_of_test = 153
    for sequence_nr in range(0,nr_of_test,1):    	

    	test_seq = run_test_sequence(sequence_nr)
